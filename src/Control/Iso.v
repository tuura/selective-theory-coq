...
Obligation 3.
  ...
  exists (x3 ∘ x1).
  rewrite comp_assoc.
  assert ((x0 ∘ x2) ∘ x3 = x0 ∘ (x2 ∘ x3))
    by (rewrite comp_assoc; trivial).
  assert ((x3 ∘ x1) ∘ (x0 ∘ x2) = x3 ∘ ((x1 ∘ x0) ∘ x2))
    by (rewrite comp_assoc; trivial).
  split.
  * rewrite H3, H1. trivial.
...
(*   unfold adjoint. *)
(*   intros A B C x. *)
(*   (* unfold adjoint; intros B C. *) *)
(*   unfold "≅". *)
(*   intros x. *)
(*   (* exists (fun (k : C ->B -> A -> C) (a : A) (b : B) => k (b, a)). *) *)
(*   (* unfold adjoint; intros. *) *)
(*   (* exists (fun (k : a * b -> _) a b => k (b, a)). *) *)
(*   exists (fun k (p : C) b a => k (snd p) (fst p)). *)
...
Proof.
  ...
  unfold "≅".
  exists (fun _ => tt).
  exists (fun (x : one) => False_rect _ H).
  extensionalize.
Qed.
