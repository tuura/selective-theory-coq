...
Program Instance Validation_Functor (c : Set) : Functor (Validation c) := {
...
Program Instance Validation_Applicative (c : Set) `{Monoid c} : Applicative (Validation c) :=
...
Program Instance Validation_Selective (c : Set) `{Monoid c}: Selective (Validation c) :=
...
Theorem validation_selective_law1_identity
  {A M : Set} `{Monoid M} {x : Validation M (Either A A)} :
